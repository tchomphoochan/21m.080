...
const gui_sketch = function(my) {
    ...
    // UPDATE KNOB VALUE
      if (elements[i].type == 'knob' || elements[i].type == 'dial'){
        if (dragging && currElement==i) {
          ...
          //console.log(elements[i].min,elements[i].max,elements[i].curve);
          let dyConverted = my.scaleOutput(dyScaled,0,1,elements[i].min,elements[i].max,elements[i].curve); // convert to 'value' units
  ...
  let UserElement = function(type,label,mapto,x,y,min,max,curve,value,prev,size,color,showLabel,showValue, bipolar, radioOptions,horizontal) {
    ...
    this.curve = curve; //#: exponential curve for output value
  ...
  my.addElement = function({type,label,mapto, x,y,min,max,curve,value,prev,size,color,showLabel,showValue,bipolar,radioOptions,horizontal}) {
    ...
    if (update == false){
      ...
      if (mapto == undefined) {mapto = 'mapto';}
      if (x == undefined) {
      ...
      elements.push(new UserElement(type,label,mapto,x,y,min,max,curve,value,prev,size,color,showLabel,showValue,bipolar,radioOptions,horizontal));
      //let UserElement = function(type,label,mapto,x,y,min,max,curve,value,prev,size,color,showLabel,showValue, bipolar, radioOptions,horizontal) {
      console.log(elements);
    }
  ...
  function redraw() { //rdrwwwwwww
    ...
    for (let i = 0; i < elements.length; i++) {
      ...
      if (elements[i].type == 'line') my.drawLine(i)
          ...
          console.log( elements[i].mapto )
          if( elements[i].mapto === 'mapto'){ console.log("no mapping") }
          else { eval(elements[i].mapto + '.rampTo(elements[i].value, 0.1)');}
